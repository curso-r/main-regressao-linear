...
Spam: $\log\Bigg(\frac{p(\text{11 exc})}{1 - p(\text{11 exc})}\Bigg) - \log\Bigg(\frac{p(\text{10 exc})}{1 - p(\text{10 exc})}\Bigg) = \beta_1$

<br/>

Inadimplência: $\log\Bigg(\frac{p(\text{empregado})}{1 - p(\text{empregado})}\Bigg) - \log\Bigg(\frac{p(\text{desempregado})}{1 - p(\text{desempregado})}\Bigg) = \beta_1$
...
Spam: $\frac{p(\text{11 exc})}{1 - p(\text{11 exc})} = \frac{p(\text{10 exc})}{1 - p(\text{10 exc})}  * e^{\beta_1}$
...
$$\log\Bigg(\frac{p(\text{11 exc})}{1 - p(\text{11 exc})}\Bigg) - \log\Bigg(\frac{p(\text{10 exc})}{1 - p(\text{10 exc})}\Bigg) = 0.040325$$

ou

$$\log\Bigg(\frac{\frac{p(\text{11 exc})}{1 - p(\text{11 exc})}}{\frac{p(\text{10 exc})}{1 - p(\text{10 exc})}}\Bigg) = 0.040325$$
...
$$\frac{p(\text{11 exc})}{1 - p(\text{11 exc})} = \frac{p(\text{10 exc})}{1 - p(\text{10 exc})}  * e^{0.040325}$$
ou

$$\frac{p(\text{11 exc})}{1 - p(\text{11 exc})} = \frac{p(\text{10 exc})}{1 - p(\text{10 exc})}  * 1.04$$
...
```{r, echo=TRUE}
gtsummary::tbl_regression(modelo)
```


```{r, echo=TRUE}
gtsummary::tbl_regression(modelo, exponentiate = TRUE)
```
...
```{r, echo=TRUE}
gtsummary::tbl_regression(churn)
```

```{r, echo=TRUE}
gtsummary::tbl_regression(churn, exponentiate = TRUE)
```
