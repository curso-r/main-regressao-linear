...
```{r, fig.height=1.2, fig.align="center", fig.width=2, echo=FALSE, message=FALSE, warning=FALSE}
# x, y, z variables
x <- mtcars$wt
y <- mtcars$disp
z <- mtcars$mpg
...
### Gráfico de Resíduos
```{r,  message = FALSE, warning=FALSE, fig.width=8, fig.height=4.5, fig.align="center"}
...
hetero_com_predicoes <- augment(lm(y ~ x, data = hetero_df))
...
hetero_log_com_predicoes <- augment(lm(log(y) ~ x, data = hetero_df))
...
```{r, fig.height=4}
cars_log_com_predicoes <- augment(lm(log(dist) ~ log(speed), data = cars))
...
- Transformações na variável resposta. log(), etc;
