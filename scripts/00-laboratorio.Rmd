...
mm <- model.matrix(flipper_length_mm ~ sex, data = penguins)

d <- cbind(na.omit(penguins)[,], mm[, ])

d %>% ggplot(aes(x = sexmale, y = flipper_length_mm)) + geom_point() +
...
penguins %>%
  group_by(species) %>%
  summarise(
    media_flipper = mean(flipper_length_mm, na.rm = TRUE)
  )
```
...
modelo_poly3 <- lm(y ~ poly(x, 3), data = dados_poly)
```
